...
the jdxld linker, then build with the environment variable `JDXLD_REFERENCE_LINKER` set to the name of
another linker. e.g.

```sh
JDXLD_REFERENCE_LINKER=ld cargo test
```

When this variable is set, each time the jdxld linker is invoked, it'll call the specified linker
