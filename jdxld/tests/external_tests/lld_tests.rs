//! Runs LLD's ELF test suite via lit with jdxld substituted for ld.lld.
...
fn run_lit_for_arch(
    ...
    let cfg_src = repo_root.join("jdxld/tests/external_tests/jdxld-lit.site.cfg.py");
    let lit_tmp = tempfile::tempdir()?;
    std::os::unix::fs::symlink(&cfg_src, lit_tmp.path().join("jdxld-lit.site.cfg.py"))?;
    std::os::unix::fs::symlink(test_dir.join("ELF"), lit_tmp.path().join("ELF"))?;

    let jdxld_bin = std::path::Path::new(env!("CARGO_BIN_EXE_jdxld"));
    let llvm_tools_dir = test_config.llvm_tools_dir.to_str().unwrap();

    // Create a fakes directory with all lld variant names pointing to jdxld.
    ...
    for linker_name in &["ld.lld", "lld-link", "ld64.lld", "wasm-ld"] {
        ...
        libjdxld::make_executable(&std::fs::File::open(&script_path)?)?;
    }
    ...
    cmd.arg("--config-prefix")
        ...
        .env("JDXLD_BIN", jdxld_bin)
        .env("JDXLD_FAKES_DIR", fakes_dir.path())
        ...
        .env("JDXLD_EMULATION", emulation)
        .env("JDXLD_LIT_CFG", test_dir.join("lit.cfg.py"));
