...
use crate::get_jdxld_test_cross;
use libjdxld::error::Context;
...
fn verify_skipped_mold_tests_still_fail(mold_test: PathBuf) -> Result {
    ...
    if output.status.success() {
        ...
        let message = if using_third_party_linker() {
            format!(
                "Test `{}` is in the skip list (fails with jdxld) but passes with '{linker}'. This indicates the failure may be jdxld-specific.\nOutput:\n{stdout}",
                mold_test.display(),
            )
        } else {
            format!(
...
// Therefore, we have to implement a similar filter for the jdxld tests as well.
fn should_skip_mold_test_by_arch(path: &Path) -> bool {
    ...
    let cross_archs = get_jdxld_test_cross().unwrap().unwrap_or_default();
