...
impl ExternalLinker {
    fn is_jdxld(&self) -> bool {
        matches!(self, ExternalLinker::Jdxld)
    }

    fn name(&self) -> &str {
        match self {
            ExternalLinker::Jdxld => "jdxld",
...
fn get_external_linker() -> &'static ExternalLinker {
    ...
    VALUE.get_or_init(|| {
        let Ok(val) = env::var("JDXLD_EXTERNAL_LINKER") else {
            return ExternalLinker::Jdxld;
        };
        let val = val.trim();
        if val.is_empty() || val.eq_ignore_ascii_case("jdxld") {
            return ExternalLinker::Jdxld;
        }
                ...
                if p.exists() {
                    return ExternalLinker::ThirdParty {
                        ...
                        path: std::fs::canonicalize(&p)
                            .expect("failed to canonicalize JDXLD_EXTERNAL_LINKER path"),
                    };
                }

                let path = which::which(val).unwrap_or_else(|_| {
                    panic!("JDXLD_EXTERNAL_LINKER={val}: not found as a file and not on PATH")
                });
        ...
        let path = search_names
            ...
            .unwrap_or_else(|| {
                panic!(
                    "JDXLD_EXTERNAL_LINKER={val}: could not find any of [{}] on PATH",
                    search_names.join(", ")
                )
            });
...
fn should_not_ignore_tests(external_test: &str) -> bool {
    let jdxld_ignore_skip: Option<Vec<String>> =
        std::env::var("JDXLD_IGNORE_SKIP").ok().map(|test_suites| {
            test_suites
                .split(',')
    ...
    jdxld_ignore_skip.is_some_and(|tests| {
