...
pub(crate) const FILES_PER_GROUP_ENV: &str = "JDXLD_FILES_PER_GROUP";
pub const REFERENCE_LINKER_ENV: &str = "JDXLD_REFERENCE_LINKER";
pub const VALIDATE_ENV: &str = "JDXLD_VALIDATE_OUTPUT";
pub const JDXLD_UNSUPPORTED_ENV: &str = "JDXLD_UNSUPPORTED";
pub const WRITE_LAYOUT_ENV: &str = "JDXLD_WRITE_LAYOUT";
pub const WRITE_TRACE_ENV: &str = "JDXLD_WRITE_TRACE";
pub const EXPERIMENTAL_PLATFORMS: &str = "JDXLD_EXPERIMENTAL_PLATFORMS";
...
/// inconsistency.
pub(crate) const WRITE_VERIFY_ALLOCATIONS_ENV: &str = "JDXLD_VERIFY_ALLOCATIONS";
...
impl Default for CommonArgs {
    fn default() -> Self {
        Self {
            ...
            print_allocations: env::var("JDXLD_PRINT_ALLOCATIONS")
                .ok()
...
impl CommonArgs {
    ...
    pub(crate) fn linker_identity(&self) -> String {
        format!("jdxld {} (compatible with GNU linkers)", self.version)
    }
...
mod tests {
    ...
    #[test]
    fn test_flavor() {
        let args = Args::new(|| ["ld.jdxld"].into_iter()).unwrap();
        assert_matches!(args, Args::Elf(_));

        let args = Args::new(|| ["ld64.jdxld"].into_iter()).unwrap();
        assert_matches!(args, Args::MachO(_));

        let mut args = Args::new(|| ["jdxld", "-flavor", "gnu"].into_iter()).unwrap();
        assert_matches!(args, Args::Elf(_));
        args.parse(|| ["jdxld", "-flavor", "gnu"].into_iter())
            .unwrap();
        assert_eq!(args.common().inputs, []);

        let args = Args::new(|| ["jdxld", "-flavor", "darwin"].into_iter()).unwrap();
        assert_matches!(args, Args::MachO(_));

        // -flavor has priority
        let args = Args::new(|| ["ld.jdxld", "-flavor", "darwin"].into_iter()).unwrap();
        assert_matches!(args, Args::MachO(_));

        let args = Args::new(|| ["ld64.jdxld", "-flavor", "gnu"].into_iter()).unwrap();
        assert_matches!(args, Args::Elf(_));

        let args = Args::new(|| ["jdxld", "-flavor", "link"].into_iter()).unwrap();
        assert_matches!(args, Args::Coff(_));

        assert!(Args::new(|| ["ld.jdxld", "-flavor", "invalid"].into_iter()).is_err());
        assert!(Args::new(|| ["ld.jdxld", "-flavor"].into_iter()).is_err());
    }
}
