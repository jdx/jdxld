...
///
/// ```
/// use libjdxld::{FileSystem, FileType, InputFileData, Linker, OutputFileData, OutputOptions};
...
///     }
///
///     fn finish(mut self) -> libjdxld::error::Result {
...
///     ) -> libjdxld::error::Result<(Self::Input, Option<Arc<File>>)> {
///         let bytes = self
///             .files
///             .lock()
///             .unwrap()
...
///             .cloned()
///             .ok_or_else(|| libjdxld::error!("No such in-memory file: {}", path.display()))?;
///         Ok((Input(bytes), None))
///     }
///
///     fn file_type(&self, path: &Path) -> libjdxld::error::Result<FileType> {
///         if self
///             .files
///             .lock()
///             .unwrap()
...
///     }
///
///     fn canonicalize(&self, path: &Path) -> libjdxld::error::Result<PathBuf> {
///         Ok(path.to_path_buf())
///     }
///
///     fn rename_file(&self, path: &Path, new_path: &Path) -> libjdxld::error::Result<()> {
...
///     }
///
///     fn remove_file(&self, path: &Path) -> libjdxld::error::Result<()> {
///         Ok(self
///             .files
///             .lock()
///             .unwrap()
...
///     ) -> libjdxld::error::Result<Self::Output> {
///         let size = usize::try_from(options.size)
///             .map_err(|_| libjdxld::error!("output is too large for this platform"))?;
///         Ok(Output {
...
///     }
///
///     fn write_auxiliary(&self, path: &Path, bytes: &[u8]) -> libjdxld::error::Result {
///         self.files
///             .lock()
///             .unwrap()
...
///         .cloned()
///         .ok_or_else(|| libjdxld::error!("linker did not create libx.so"))?;
...
impl FileSystem for CachingFileSystem {
    ...
    fn open_input(
        ...
        let (input, file) = if let Some(cached) = cached {
            (cached.clone(), Arc::clone(&cached.file))
        } else {
            let file = Arc::new(
                File::open(&cache_key)
...
mod caching_tests {
    ...
    fn reuses_unchanged_input_and_reloads_changed_input() {
        ...
        assert!(Arc::ptr_eq(&first.input.file, &second.input.file));
