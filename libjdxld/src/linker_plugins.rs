...
const SAVE_VAR_NAME: &str = "JDXLD_SAVE_PLUGIN_OUTPUTS";
...
impl LoadedPlugin {
    fn new(plugin_path: &Path, args: &ElfArgs, get_symbols_v3: GetSymbols) -> Result<LoadedPlugin> {
        ...
        if cfg!(target_feature = "crt-static") {
            bail!(
                "Linker plugins cannot be used when jdxld was built as a statically linked binary"
            );
        }
        ...
        transfer_vector.push(LdPluginTv {
            ...
            value: jdxld_plugin_message_callback as *const () as usize,
        });
...
unsafe extern "C" {
    ...
    /// then calls `jdxld_handle_plugin_message` with the resulting string. Defined in
    /// `plugin_message_shim.c`.
    fn jdxld_plugin_message_callback(level: libc::c_int, fmt: *const libc::c_char, ...);
}

/// Called by the C shim `jdxld_plugin_message_callback` with the already-formatted message string.
/// The `no_mangle` is required so the C shim can link against it by name.
#[unsafe(no_mangle)]
extern "C" fn jdxld_handle_plugin_message(level: libc::c_int, message: *const libc::c_char) {
