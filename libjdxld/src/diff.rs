//! If the environment variable `JDXLD_REFERENCE_LINKER` is set, then once we've finished linking,
...
//! enable writing of trace and layout files by the jdxld linker, which allow additional information
//! to be added to the diff outputs.
//!
//! For this to work, the linker-diff binary needs to be installed in the same directory as jdxld.
...
fn run_diff(paths: &BinPaths) -> Result {
    ...
    if !linker_diff_path.exists() {
        bail!("linker-diff binary needs to be in the same directory as jdxld")
    }
