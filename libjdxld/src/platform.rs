...
pub(crate) trait Args: std::fmt::Debug + Send + Sync + 'static {
    ...
    fn warn_unsupported(&self, opt: &str) -> Result {
        use crate::args::JDXLD_UNSUPPORTED_ENV;

        let message = format!("{opt} is not yet supported");

        match env::var(JDXLD_UNSUPPORTED_ENV).unwrap_or_default().as_str() {
            "warn" | "" => self.warning(message),
            "ignore" => {}
            "error" => bail!("{message}"),
            other => bail!("Unsupported value for {JDXLD_UNSUPPORTED_ENV}={other}"),
        }
        Ok(())
    }
