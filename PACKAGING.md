# Recommendations for Packaging jdxld linker

## Binaries

This repository consists of jdxld linker and linker-diff binaries. We recommend providing the `jdxld`
binary and symlink `ld.jdxld` pointing to it, as this enables `-fuse-ld=jdxld` usage with Clang.
Linker-diff is mainly a tool to aid jdxld development, so you most likely don't want to package it.
...
jdxld has two optional build-time features:
...
- `mimalloc` (disabled by default) – build and use Mimalloc as the allocator instead of the system
  one. It performs marginally worse than Glibc in jdxld's case, but much better than Musl.
...
used if absent) and `JDXLD_TEST_IGNORE_FORMAT`.
...
with `run_all_diffs` option, it's meant mostly for jdxld development and may cause false positives.

Setting `JDXLD_TEST_IGNORE_FORMAT` disables format checks of C/C++ source files in tests which you
might prefer because your `clang-format` may give different results.

## Issues

If you, or the users of your package, encounter any issues or pain points with jdxld, don't hesitate
